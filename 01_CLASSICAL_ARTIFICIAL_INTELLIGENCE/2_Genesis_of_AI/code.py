"""
Chapter 1.2: Genesis of AI - The Environment-Agent Loop
...
Perception -> Decision -> Action loop, is implemented with perceive, decide, and act.

Symbol manipulation is enough. 
Actions like "increment"/"decrement" are symbolic manipulations of state.

This chapter establishes the foundational abstractions (Environment and Agent)
that will be reused throughout the entire LINEAGE project.
"""
...
class Environment(ABC):
    """An abstraction for environments where agents operate.
    
    This concept originates from classical AI and remains relevant
    in modern reinforcement learning and agentic systems.
    """
...
class Agent(ABC):
    """An abstraction for agents that perceive, decide, and act.
    
    The perception-decision-action loop is fundamental to AI,
    from classical symbolic agents to modern LLM-based agents.
    """
    ...
    @abstractmethod
    def decide(self):
        """Decide on the next action based on perceived state."""
        pass
...
4. We use a while loop to keep checking if the agent has effected the environment enough that the state has now reached the intended goal. 
